...
# Set the environment, landscape, and network variable palette
...
## Set target CRS
target.crs <- "+proj=moll"

## Set dataframe of site coordinates for plotting after CRS transformation
site.coordinates.sf <- final.data %>%
  select(ecoregion, site.lat, site.long) %>%
  st_as_sf(coords = c("site.long", "site.lat"), crs = 4326) %>%
  st_transform(crs = target.crs)

## Basic continent map
# Get all countries in North America
base.contintent.map.data <- ne_countries(
  scale = "large",
  type = "countries",
  ...
  st_crop(xmin = -175, xmax = -30, ymin = 2.5, ymax = 85)

# Set the base projection for the continent map
inset.continent.map <- ggplot(data = base.contintent.map.data) +
  geom_sf(fill = "antiquewhite") +
  coord_sf(crs = st_crs(target.crs), expand = FALSE) +
  theme(
...
## Set the site map
site.map <- ggplot(data = base.contintent.map.data %>% filter(adm0_a3 == c("CAN", "MEX", "USA"))) +
  geom_sf(fill = "antiquewhite") +
  coord_sf(
    crs = st_crs(target.crs),
    xlim = c(-11850000, -4500000), ylim = c(2000000, 6000000),
    expand = FALSE
  ) +
  geom_point(
    data = site.coordinates.sf,
    size = 1.5, alpha = 0.5,
    aes(colour = ecoregion, geometry = geometry),
    stat = "sf_coordinates"
  ) +
...
## Combine the inset and site map
combined.site.map <- ggdraw(site.map) +
  draw_plot(inset.continent.map, width = 0.325, height = 0.325, x = 0.5875, y = 0.165)
```

```{r Save the Site Map, include = FALSE}
ggsave("figure_1-site_map-base.pdf",
  plot = combined.site.map,
  device = "pdf",
  path = "figures/base/",
  width = 12,
  height = 9,
...
DisEQ.figure <- ggerrorplot(
  ...
  scale_x_discrete(
    limits = c("WMT", "XER", "NPL", "SPL", "TPL", "UMW", "NAP", "SAP", "CPL"),
...
## Export the figure
ggsave("figure_S2-vector_components-base.pdf",
...
knitr::include_graphics(paste0(getwd(), "/figures/edited/figure_S2-vector_components-edited.pdf"))
```
...
## Re-order variables for the facet plots
trait.diversity.long$diversity.measure <- fct_relevel(
  trait.diversity.long$diversity.measure,
  levels = c("FRic", "FEve", "FDiv", "FDis")
)
```
...
trait.diversity.figure <- ggerrorplot(
  ...
  color = "ecoregion",
  xlab = "Ecoregion",
  ylab = "Trait Diversity Measurement",
  palette = viridis.ecoregion,
  add = "boxplot",
  ggtheme = theme_pubr(),
  legend = "top"
...
```{r Trait-by-Environment Figure, echo = TRUE}
trait.by.environment.figure <- ggbarplot(
...
## Export the figure
ggsave("figure_6-trait_by_environment-base.pdf",
  plot = trait.by.environment.figure,
  device = "pdf",
  path = "figures/base/",
  width = 12,
  height = 12,
...
knitr::include_graphics(paste0(getwd(), "/figures/edited/figure_6-trait_by_environment-edited.pdf"))
```
...
## Export the figure
ggsave("figure_S3-filtering_by_trait_diversity-base.pdf",
...
## Export the figure
ggsave("figure_S4-mismatch_by_trait_diversity-base.pdf",
...
knitr::include_graphics(paste0(getwd(), "/figures/edited/figure_S3-filtering_by_trait_diversity-edited.pdf"))
```
...
knitr::include_graphics(paste0(getwd(), "/figures/edited/figure_S4-mismatch_by_trait_diversity-edited.pdf"))
```
...
## Export the figure
ggsave("figure_S5-DisEQ_regression-base.pdf",
...
knitr::include_graphics(paste0(getwd(), "/figures/edited/figure_S5-DisEQ_regression-edited.pdf"))
```
...
pdf("figures/base/figure_S6A-community_composition-base.pdf", width = 8, height = 6)

## NMDS plot (axes 1 and 2)
axes.1_2 <- ordiplot(BC.NMDS,
  choices = c(1, 2), type = "none", display = "sites",
...
mtext(
  text = "Stress = 0.199", side = 3, line = -1.5, adj = 0.95,
  cex = 1.05
)
...
## Set figure dimensions
pdf("figures/base/figure_S6B-community_composition-base.pdf", width = 8, height = 6)

## NMDS plot (axes 1 and 3)
axes.1_3 <- ordiplot(BC.NMDS,
  choices = c(1, 3), type = "none", display = "sites",
  xlim = c(-1.5, 1.5), ylim = c(-1.5, 1.5), xlab = "NMDS 1", ylab = "NMDS 3"
)
ordiellipse(axes.1_3,
  groups = NMDS.groups,
  draw = "lines", col = viridis.ecoregion, label = FALSE, lwd = 5
)
...
knitr::include_graphics(paste0(getwd(), "/figures/edited/figure_S6-community_composition-edited.pdf"))
```
